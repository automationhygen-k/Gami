...
- **GitHub Actions final APK build** in `.github/workflows/android-ci.yml` runs as a single PR/manual workflow (with concurrency cancel), performs one compile command (`:app:assembleDebug`), and publishes one installable artifact `Gami-final.apk` by resolving the produced debug APK path dynamically; texture validation is still enforced via `preBuild` dependency.
