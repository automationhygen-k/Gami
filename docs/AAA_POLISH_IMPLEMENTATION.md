...
- **GitHub Actions final APK build** in `.github/workflows/android-ci.yml` runs as a single PR/manual workflow (with concurrency cancel), sets up Android SDK + Gradle, validates textures, assembles a debug APK, and publishes one installable artifact `Gami-final.apk`.
